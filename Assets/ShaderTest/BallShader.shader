	...
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
				...
				float pointDistance = distance((_Value1, _Value2), IN.uv);
				float pointDistance2 = distance((_Value7, _Value8), IN.uv);

				fixed4 temp;
				//Weißer Punkt und Farbe
				temp.rgb = lerp(_ColorPoint.rgb, lerp(_Color1.rgb, _Color2.rgb, IN.uv.x), pointDistance2 / 0.5);

				 //temp.rgb = lerp(_Color1.rgb, _Color2.rgb, IN.uv.x);
				 //temp.rgb = lerp(_ColorPoint, temp.rgb, (clamp((pointDistance + _Value3) * _Value4, 0, 1) + _Value5) * _Value6);
				
				 ////Fresnel Effect
				 //temp.rgb = lerp(temp.rgb, _ColorBorder, (clamp((pointDistance2 + _Value9) * _Value10, 0, 1) + _Value11) * _Value12);
